...
def calc_ndcg(queries, retrievals, query_labels, retrieval_labels, top_k):
    """
    Calculate Normalized Discounted Cumulative Gain
    Args:
        queries: query embeddings (n_query × dim)
        retrievals: retrieval embeddings (n_retrieval × dim)
        query_labels: ground truth labels for queries (n_query × n_classes)
        retrieval_labels: ground truth labels for retrievals (n_retrieval × n_classes)
        top_k: calculate NDCG@top_k
    Returns:
        mean NDCG score across all queries
    """
    scores = []
    similarity = queries @ retrievals.T  # cosine similarity
    
    for i in range(len(queries)):
        # Get relevance scores (1 for relevant, 0 for irrelevant)
        relevance = (query_labels[i] @ retrieval_labels.T).astype(float)
        relevance[relevance > 0] = 1  # binary relevance
        ...
        for j, idx in enumerate(top_indices):
            dcg += relevance[idx] / np.log2(j + 2)  # j starts from 0
        
        # Calculate IDCG (perfect ranking)
        ideal_relevance = np.sort(relevance)[-top_k:][::-1]
        idcg = 0
