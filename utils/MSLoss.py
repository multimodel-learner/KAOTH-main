...
class MSLoss(nn.Module):
    def __init__(self,batch_norm, temperature=0.3, totalepoch=100, self_paced=True):
        super(MSLoss, self).__init__()
        self.temperature = temperature
        self.totalepoch = totalepoch
        self.self_paced = self_paced
        self.batch_norm = batch_norm
    def forward(self,batch_norm, image_feature, text_feature, labels=None, epoch=0):
        ...
        if self.self_paced:
            if epoch <= int(self.totalepoch/batch_norm):
                delta = epoch / int(self.totalepoch/batch_norm)
            else:
                delta = 1
